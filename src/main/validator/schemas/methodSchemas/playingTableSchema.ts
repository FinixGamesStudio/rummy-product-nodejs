...
const playingTableSchema = Joi.object()
    .keys({
        ...
        isUseBot: Joi.boolean().required().description('user or bot'),
        ...
        isCreateRoom: Joi.boolean().optional(),
        isReferralCode: Joi.string().allow("").optional(),

        rummyType: Joi.string().allow("").description('rake Value'),
        moneyMode: Joi.string().allow("").description('rake Value'),

        botType : Joi.string().allow("").description('botType'),
        botJoinTimer : Joi.number().description('botJoinTimer'),
        declareTurnCount : Joi.number().description('botdeclareTurnCount'),
    })
    .unknown(true);
