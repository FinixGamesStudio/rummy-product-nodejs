import { BOT_CONSTANT, ERROR_TYPE, EVENTS, GRPC_ERROR_REASONS, MESSAGES, NUMERICAL, RUMMY_TYPES } from "../../../../constants";
...
import botWinTimerQueue from "../../../scheduler/queues/BOT/botWinTimer.queue";
...
async function startRound(
    ...
    try {
        ...
        logger.info("------>> startRound :: tableData :: ", tableData);
        ...
        logger.info("------>> startRound :: tableData.botType :: ", tableData.botType);

        if(tableData.botType === BOT_CONSTANT.BOT_TYPES.EXCELLENT){

            let botWinTimer = await getRandomNumber(50,100) ? await getRandomNumber(50,100) : 100
            logger.info("------>> startRound :: botWinTimer :: ", botWinTimer);
           let finalBotWinTimer = Math.floor(botWinTimer / NUMERICAL.TWO)
           logger.info("----->> startRound :: finalBotWinTimer :: ", finalBotWinTimer)

           let totalFinalBotWinTime :number = finalBotWinTimer * roundTableData.currentPlayer
           logger.info("----->> startRound :: totalFinalBotWinTime :: ", totalFinalBotWinTime)
           

            /* bot win timer start*/
            logger.info("----->> botWinTimerQueue :: START ");
            await botWinTimerQueue({
                timer : totalFinalBotWinTime * NUMERICAL.THOUSAND,
                tableId
            })
        }
