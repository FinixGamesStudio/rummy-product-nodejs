...
import botAutoDeclareQueue from "../../../../scheduler/queues/BOT/botAutoDeclare.queue";
    ...
    try {
        if (
            ...
            if (totalPoints === NUMERICAL.ZERO) {
                ...
                let time = await getRandomNumber(NUMERICAL.THREE , NUMERICAL.TEN)
                logger.info(`----->> autoDeclareFinishDeckUser :: time ::`, time);

                await botAutoDeclareQueue({
                    timer: time * NUMERICAL.THOUSAND,
                    tableId,
                    currentRound
                });
            } else {
                ...
                let queueKey: string =
                tableData.isCreateRoom && tableData.isReferralCode != ""
                    ? `${gameType}:${tableData.isReferralCode}`
                    : `${gameType}:${gameId}:${lobbyId}`;

                logger.info(`----->> autoDeclareFinishDeckUser :: queueKey ::`, queueKey);
