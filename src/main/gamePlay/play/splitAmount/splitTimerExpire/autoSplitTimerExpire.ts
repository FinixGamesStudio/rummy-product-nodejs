    ...
    try {
        ...
        for (const userId of userIds) {
            ...
            if (playerData && playerData.playingStatus === PLAYER_STATE.SPLIT_AMOUNT) {
                logger.info("-------->> autoSplitTimerExpire :: userData :: 1", userData);

                if(userData.isCash){
                    userData.cash += playerData.splitDetails.amount;
                }else{
                    userData.coins += playerData.splitDetails.amount;
                }
                // userData.balance += playerData.splitDetails.amount;

                await setUser(playerData.userId, userData)
                logger.info("-------->> autoSplitTimerExpire :: userData :: 2 ", userData);
        ...
        logger.info("------>> ------>> autoSplitTimerExpire : : :: apiData :: ", apiData);

         // creadit amount for winner players
         const winnerApiData  = await winnerScoreHandle(apiData, tableData.gameId);
         logger.info("------>> ------>> autoSplitTimerExpire : : :: winnerApiData :: ", winnerApiData);
