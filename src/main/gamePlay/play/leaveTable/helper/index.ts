...
const userLeaveOnWaitForPlayer = async (
  ...
  try {
    ...
    await removeQueue(`${queueKey}`, playingTable._id);
...
const userLeaveOnWaitingPlayer = async (
  ...
  playingTable.isCreateRoom && playingTable.isReferralCode != ""
      ...
      logger.info("----->> userLeaveOnWaitingPlayer :: queueKey :::",queueKey);
    ...
    if (roundPlayingTable.totalPlayers < playingTable.minPlayerForPlay) {
      ...
      if (roundPlayingTable.totalPlayers < NUMERICAL.ONE) {
        ...
        if(!userData.isCreateRoom){
          logger.info(`----->> userLeaveOnWaitingPlayer :: userData.isCreateRoom ::  ::`, userData.isCreateRoom);
          const userInfo = await setOldTableIdsHistory(userData, playingTable._id);
          userData.oldTableId = userInfo.oldTableId;
          logger.info(`----->> userLeaveOnWaitingPlayer :: userInfo :: ::`, userInfo);
        }

        await setUser(userId, userData)
        logger.info(`----->> userLeaveOnWaitingPlayer :: userData after 2----> :: ::`, userData);
      ...
      logger.info(` after :: <<< :: userLeaveOnWaitingPlayer :: >>>  queueKey ====22222 :::>> `, queueKey);
      ...
      logger.info(`------>>  <<< :: userLeaveOnWaitingPlayer :: >>>  :: :: privateTableKeyStatus :: `, privateTableKeyStatus);

     const findTable: privateTableKey = await DB.findOne(privateTableKeyModel, {
         query: { isReferralCode },
       });
       logger.info("----->> <<< :: userLeaveOnWaitingPlayer :: >>>  :: :: findTable :=>> ", findTable);

      // referralCode remove in Redis
       await removeReffCode(userData.isReferralCode)
...
const userLeaveOnLock = async (
  ...
  try {
    ...
    if (roundPlayingTable.tableState === TABLE_STATE.ROUND_TIMER_STARTED) {
      ...
      logger.info(` after :: <<< :: userLeaveOnLock :: >>>  queueKey :::>> `, queueKey);
