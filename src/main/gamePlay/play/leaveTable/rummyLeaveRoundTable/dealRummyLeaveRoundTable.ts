  ...
  try {
    ...
    if (
      ...
      if (playerLeave && flag) {
        ...
        logger.info(
          "----->> dealRummyLeaveRoundTable :: totalUserCards ::",
          totalUserCards
        );

        if (totalUserCards === NUMERICAL.FOURTEEN) {
          logger.info(
            "----->> dealRummyLeaveRoundTable :: cardNotDiscardCard ::"
          );
        ...
        logger.info(
          "----->> dealRummyLeaveRoundTable :::: 1 :::: roundTableData ::",
          roundTableData
        );
        logger.info(
          "----->> dealRummyLeaveRoundTable :::: 1 :::: playerData ::",
          playerData
        );
        let winPrice =
        ...
        logger.info(
          "------->> dealRummyLeaveRoundTable :: formatLeaveTableInfo ::",
          formatedRes
        );
        ...
        logger.info(
          " ------>> ------>> dealRummyLeaveRoundTable  :: findUser ::>> ",
          findUser
        );
        if (!findUser) {
        ...
        if (roundTableData.tableState != TABLE_STATE.DISPLAY_SCOREBOARD) {
          ...
          logger.info(
            " dealRummyLeaveRoundTable :: updatedUser :: ==>> ",
            updatedUser
          );
        }
      ...
      logger.warn(
        "-------->> dealRummyLeaveRoundTable :: player state ::",
        roundTableData.seats[`s${playerData.seatIndex}`].userStatus
      );
      logger.warn(
        "-------->> dealRummyLeaveRoundTable :: player state already left or lost"
      );
