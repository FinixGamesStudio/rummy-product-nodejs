import { BOT, ERROR_TYPE, EVENTS, GRPC_ERROR_REASONS, MESSAGES, NUMERICAL, PLAYER_STATE, TABLE_STATE } from "../../../../constants";
...
async function leaveTable(
    ...
    try {
        ...
        // logger.info(`----->> leaveTable ::-------->> !userData.isCreateRoom ::  ::`,userData.isCreateRoom);
        if(userData.isCreateRoom){
            logger.info(`----->> leaveTable :: userData.isCreateRoom IF ::  ::`, userData.isCreateRoom);
    ...
    if (leaveTableLocks) {
      ...
      //    await botLeave(tableId,currentRound)
      const roundTablePlayData: roundTableIf = await getRoundTableData(
        tableId,
        currentRound
      );
      logger.info(
        `----->> leaveTable :: roundTablePlayData :1:`,
        roundTablePlayData
      );
      if (roundTablePlayData) {
        logger.info(
          `----->> leaveTable :: Bot Leave :: roundTablePlayData.seats :: --> `,
          roundTablePlayData.seats
        );
        ...
        for await (const key of Object.keys(roundTablePlayData.seats)) {
          if (Object.keys(roundTablePlayData.seats[key]).length > 0) {
            if (
              roundTablePlayData.seats[key].isBot &&
              roundTablePlayData.seats[key].userStatus == PLAYER_STATE.PLAYING
            ) {
              botUserCounter++;
            } else if (
              !roundTablePlayData.seats[key].isBot &&
              roundTablePlayData.seats[key].userStatus != PLAYER_STATE.LEFT
            ) {
              realUserCounter++;
            }
          }
        ...
        logger.info(
          `--->> leaveTable :: Bot Leave :: realUserCounter :: --> `,
          realUserCounter
        );
        logger.info(
          `--->> leaveTable :: Bot Leave :: botUserCounter :: --> `,
          botUserCounter
        );
        ...
        if (realUserCounter === 0 && botUserCounter === 0) {
          ...
          logger.info(`--->> leaveTable :: Bot Leave :: IN --> `);
            ...
            if (Object.keys(roundTablePlayData.seats[key]).length > 0) {
              if (roundTablePlayData.seats[key].userStatus != PLAYER_STATE.LEFT) {
                if (roundTablePlayData.seats[key].isBot) {
                  ...
                  logger.info(
                    `----->> leaveTable :: roundTableData.tableState ::`,
                    roundTableData?.tableState
                  );
        ...
        logger.info(`----->> leaveTable :: roundTablePlayData Not Found !`);
      }
