...
import botTurnTimerQueue from "../../../scheduler/queues/BOT/botTurnTimer.queue";
...
export async function nextPlayerTurn(
    ...
    try {
        ...
        logger.info("----->> nextPlayerTurn :: tableGamePlay :: ", tableGamePlay)

        roundTableData.turnCount += 1;
        roundTableData.isSecondaryTurn = false;

        let declareTurnCount = Math.floor(tableGamePlay.declareTurnCount / NUMERICAL.TWO)
        logger.info("----->> nextPlayerTurn :: declareTurnCount :: ", declareTurnCount)

        let totalDeclareTurnCount :any = NUMERICAL.ZERO
        if(roundTableData.currentPlayer >= NUMERICAL.FIVE){
            totalDeclareTurnCount = getRandomNumber(NUMERICAL.TWENTY , NUMERICAL.TWENTY_FIVE)
        }else{
            totalDeclareTurnCount = declareTurnCount * roundTableData.currentPlayer
        }
        logger.info("----->> nextPlayerTurn :: totalDeclareTurnCount :: ", totalDeclareTurnCount)

        if((roundTableData.opendDeck.length - NUMERICAL.ONE) >= totalDeclareTurnCount){
        logger.info("----->> nextPlayerTurn :: isBotWin flag true :: ")
