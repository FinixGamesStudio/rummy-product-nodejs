...
async function checkRoundWiner(
    ...
    try {
        ...
        logger.info(
            `----->> checkRoundWiner :: watchingPlayerCount ::`,
            watchingPlayerCount
          );
        ...
        if (roundTableData.totalPlayers === NUMERICAL.ZERO) {
            ...
            logger.info(`------>>  checkRoundWiner :: :: privateTableKeyStatus :: `, privateTableKeyStatus);

            const findTable: privateTableKey = await DB.findOne(privateTableKeyModel, {
                query: { isReferralCode },
              });
              logger.info("----->> checkRoundWiner :: :: findTable :=>> ", findTable);
