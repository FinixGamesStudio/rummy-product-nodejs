...
import { dealRummyWinApi } from "../../common/dealRummyApiRespose";
...
export async function dealRummyScoreBoard(
  ...
  try {
    ...
    logger.info(
      "---------->> dealRummyScoreBoard :: playersInfo :: ",
      playersInfo
    );
    logger.info(
      "---------->> dealRummyScoreBoard :: formatedRes :: ",
      formatedRes
    );
    ...
    if (
      ...
      logger.info(
        `---------->> dealRummyScoreBoard :: roundTableData.tableState :: ${roundTableData.tableState} ::  roundTableData.totalPlayers :: ${roundTableData.totalPlayers}`
      );
      ...
      logger.info(
        `---------->> dealRummyScoreBoard :: formatedRes :: ${roundTableData.tableState} :: `,
        formatedRes
      );
      logger.info(
        `---------->> dealRummyScoreBoard :: finalWinnerPlayerData :: `,
        finalWinnerPlayerData
      );

      const playerData = await getPlayerGamePlay(
        finalWinnerPlayerData.userId,
        tableId
      );
      ...
      if (!allDeclared && isNextRound) {
        ...
        logger.info(
          "---------->> dealRummyScoreBoard :: finalWinner :: 1 ",
          finalWinner
        );
        let playerCount: number = NUMERICAL.ZERO;
        ...
        logger.info(
          "---------->> dealRummyScoreBoard :: playersInfo :: JSON :: 1 ",
          playersInfo
        );
        ...
        for await (const player of formatedRes.scoreBoradTable) {
          if (finalWinner.isMultiWinner) {
            for await (const drawnPlayer of finalWinner.mulltiWinnerPlayerData) {
              if (drawnPlayer.userId === player.userId) {
                ...
                formatedRes.scoreBoradTable[playerCount].Status =
                  PLAYER_STATE.DRAW;
                // formatedRes.scoreBoradTable[playerCount].Status = PLAYER_STATE.WIN;
              }
            }
          ...
          if (
            ...
            const playerData = await getPlayerGamePlay(
              finalWinner.winnerPlayerData.userId,
              tableId
            );
            playerData.isWinner = true;
            await setPlayerGamePlay(
        ...
        logger.info(
          "---------->> dealRummyScoreBoard :: playersInfo :: JSON :: 2",
          playersInfo
        );
      ...
      if (allDeclared) {
        ...
        logger.info(
          "---------->> dealRummyScoreBoard :: finalWinner :: 2 ",
          finalWinner
        );
      ...
      if( currentRound == tableData.dealType ||
        roundTableData.totalPlayers === NUMERICAL.ONE){
         logger.info("------------------->> :: :: :: dealRummyScoreBoard :: :: :: ---------------------->>");
          await dealRummyWinApi(roundTableData,tableId)
      }
      ...
      const finalWinnerDeclare = await dealRummyCheckFinalWinerInScoreBoard(
        playersInfo,
        currentRound
      );
      logger.info(
        "---------->> dealRummyScoreBoard :: finalWinnerDeclare :: 3",
        finalWinnerDeclare
      );

      playersInfo = JSON.parse(JSON.stringify(playersInfo));
      logger.info(
        "---------->> dealRummyScoreBoard :: playersInfo :: JSON :: ",
        playersInfo
      );
      ...
      logger.info(
        "---------->> dealRummyScoreBoard :: allPlayerStatus :: ",
        allPlayerStatus
      );
      ...
      logger.info(
        "---------->> dealRummyScoreBoard :: isAllPlayerDeclared :: ",
        !isAllPlayerDeclare
      );
      ...
      for await (const mulltiWinnerPlayer of finalWinnerDeclare.mulltiWinnerPlayerData) {
        finalWinnerDeclare.mulltiWinnerPlayerData[
          mulltiWinnerPlayerIndex
        ].Status = PLAYER_STATE.DRAW;
      ...
      for await (const player of formatedRes.scoreBoradTable) {
        if (!isAllPlayerDeclare) {
          ...
          if (
            finalWinnerDeclare.isFinalWinner &&
            player.Status === PLAYER_STATE.WIN_ROUND &&
            player.userId !== finalWinnerDeclare.winnerPlayerData.userId
          ) {
          ...
          if (
            ...
            const playerData = await getPlayerGamePlay(
              finalWinnerDeclare.winnerPlayerData.userId,
              tableId
            );
            playerData.isWinner = true;
            await setPlayerGamePlay(
      ...
      for await (const player of playersInfo) {
        logger.info(
          "----->> dealRummyScoreBoard :: player.status ",
          player.Status
        );
        if (
          ...
          declaredPlayerData = JSON.parse(
            JSON.stringify(declaredPlayerDataClone)
          );
          if (roundWinnerIScoreboardIndex !== NUMERICAL.MINUS_ONE) {
            formatedRes.scoreBoradTable[roundWinnerIScoreboardIndex].Status =
              PLAYER_STATE.LOST;
          }
          if (player.tableId === tableId) {
            if (userID) {
              logger.info(
                "----->> dealRummyScoreBoard :: User ID availble: " +
                  player.userId
              );
              if (player.userId === userID) {
                logger.info(
                  "---------->> dealRummyScoreBoard :: players 2 :: userID ::",
                  player.userId
                );
                logger.info(
                  "---------->> dealRummyScoreBoard :: players 2 :: ",
                  player
                );
                commonEventEmitter.emit(EVENTS.SCORE_BORAD, {
                  socket: player.socketId,
                ...
                logger.info(
                  "----->> dealRummyScoreBoard :: already display scoreBoard "
                );
                logger.info(
                  "----->> dealRummyScoreBoard :: declaredPlayerData :: ",
                  declaredPlayerData[0]
                );
                logger.info(
                  "----->> dealRummyScoreBoard :: dealWinnerIndex :: ",
                  dealWinnerIndex
                );
                ...
                if (!isAllPlayerDeclare) {
                  ...
                  logger.info(
                    "----->> dealRummyScoreBoard :: samePlayerIndex :: >> ",
                    samePlayerIndex
                  );

                  if (samePlayerIndex !== NUMERICAL.MINUS_ONE) {
                    finalWinnerDeclare.mulltiWinnerPlayerData.splice(
                      samePlayerIndex,
                      NUMERICAL.ONE
                    );
                  ...
                  if (finalWinnerDeclare.isFinalWinner) {
                    ...
                    if (
                      playersInfo[dealWinnerIndex].userId ===
                      finalWinnerDeclare.winnerPlayerData.userId
                    ) {
                      logger.info(
                        "----->> dealRummyScoreBoard :: dealWiner and finnal winner same"
                      );
                      // if declare Player is win player
                      if (
                        declaredPlayerData[0].userId ===
                        finalWinnerDeclare.winnerPlayerData.userId
                      ) {
                        otherWinner = false;
                        declaredPlayerData[0].Status = PLAYER_STATE.WIN;
                        finalWinnerDeclare.winnerPlayerData.Status =
                          PLAYER_STATE.WIN;
                      } else {
                        ...
                        finalWinnerDeclare.winnerPlayerData.Status =
                          PLAYER_STATE.WIN;
                        declaredPlayerData[NUMERICAL.ZERO].Status =
                          PLAYER_STATE.LOST;
                      }
                    }
                    // dealWiner and finnal winner not same
                      ...
                      logger.info(
                        "----->> dealRummyScoreBoard :: dealWiner and finnal winner not same"
                      );
                      ...
                      if (
                        ...
                        otherWinner = true;
                        flage = false;
                        declaredPlayerData[NUMERICAL.ZERO] = JSON.parse(
                          JSON.stringify(playersInfo[dealWinnerIndex])
                        );
                        declaredPlayerData[NUMERICAL.ZERO].Status =
                          PLAYER_STATE.LOST;
                      ...
                      // other player declare
                        ...
                        declaredPlayerData[NUMERICAL.ZERO].Status =
                          PLAYER_STATE.LOST;
                        declaredPlayerData[NUMERICAL.ONE] = JSON.parse(
                          JSON.stringify(playersInfo[dealWinnerIndex])
                        );
                        declaredPlayerData[NUMERICAL.ONE].Status =
                          PLAYER_STATE.LOST;
                      }
                    }
                  }
                ...
                logger.info(
                  "----->> dealRummyScoreBoard :: finalWinnerDeclare ",
                  finalWinnerDeclare
                );
                logger.info(
                  "----->> dealRummyScoreBoard :: finalWinnerDeclare.mulltiWinnerPlayerData ",
                  finalWinnerDeclare.mulltiWinnerPlayerData
                );
                logger.info(
                  "----->> dealRummyScoreBoard :: otherWinner ",
                  otherWinner
                );
                logger.info(
                  `----->> dealRummyScoreBoard :: otherWinner :: ${otherWinner} :: isMultiWinner :: ${
                    finalWinnerDeclare.isMultiWinner
                  } :: !isAllPlayerDeclare :: ${!isAllPlayerDeclare} :: `
                );
                logger.info(
                  `----->> dealRummyScoreBoard :: winnerPlayerData.userId === declaredPlayerDatauserId :: ${
                    finalWinnerDeclare.winnerPlayerData.userId ===
                    declaredPlayerData[0].userId
                  } :: finalWinnerDeclare.isFinalWinner :: ${
                    finalWinnerDeclare.isFinalWinner
                ...
                logger.info(
                  "----->> dealRummyScoreBoard :: declaredPlayerData[0] ",
                  declaredPlayerData[0]
                );

                const formatRes = !finalWinnerDeclare.isDealOver
                  ? await formatDealRummyDeclareInScoreboardInfo(
                      declaredPlayerData[0],
                      roundWinner
                    )
                  : finalWinnerDeclare.isMultiWinner &&
                  ...
                  ? await formatDealRummyDeclareInScoreboardInfo(
                      ...
                      [
                        ...finalWinnerDeclare.mulltiWinnerPlayerData,
                        {
                          ...playersInfo[dealWinnerIndex],
                          Status: PLAYER_STATE.LOST,
                        },
                      ]
                    )
                  : finalWinnerDeclare.isMultiWinner && !isAllPlayerDeclare
                  ? await formatDealRummyDeclareInScoreboardInfo(
                      declaredPlayerData[0],
                      finalWinnerDeclare.mulltiWinnerPlayerData
                    )
                  ...
                  ? await formatDealRummyDeclareInScoreboardInfo(
                      ...
                      [
                        {
                          ...finalWinnerDeclare.winnerPlayerData,
                          Status: PLAYER_STATE.WIN,
                        },
                        declaredPlayerData[NUMERICAL.ONE],
                      ]
                    )
                  : finalWinnerDeclare.isFinalWinner &&
                    !isAllPlayerDeclare &&
                  ...
                  : finalWinnerDeclare.isFinalWinner && !isAllPlayerDeclare
                  ? await formatDealRummyDeclareInScoreboardInfo(
                      declaredPlayerData[0],
                      [finalWinnerDeclare.winnerPlayerData]
                    )
                ...
                logger.info(
                  "---------->> dealRummyScoreBoard :: already display scoreBoard :: userID",
                  player.userId
                );
