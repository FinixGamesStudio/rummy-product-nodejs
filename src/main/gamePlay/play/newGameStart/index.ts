...
export async function newGameStart(
    ...
    try {
        ...
        const { dealType, newTableId, totalPlayers, totalRounds, gameId, lobbyId, gameType, currentRound, minPlayerForPlay,isCreateRoom,isReferralCode } = await reTableConfig(oldTableId);
        ...
        let queueKey: string =
        isCreateRoom && isReferralCode != ""
            ? `${gameType}:${isReferralCode}`
