...
async function removeQueue(
    ...
    try {
        logger.info(
            `----------->> removeQueue ::  key : ${key} :: tableId :: ${tableId} `
          );
        const newKey = `${PREFIX.QUEUE}:${key}`
        let tables: string[] = await getTableQueue(newKey);
        logger.info("--- removeQueue :: tables :: ", tables);
        logger.info("--- removeQueue :: newKey :: ", newKey);
        ...
        logger.info("--- removeQueue :: tableIdQueue :: ", tableIdQueue);
        logger.info(
          "--- removeQueue :: tableIdQueue length :: ",
          tableIdQueue.length
        );
...
async function getQueue(
    ...
    try {
        logger.info(
            `----------->> getQueue ::  key : ${key} :: oldTableIds :: ${oldTableIds} `
          );
      
        const newKey = `${PREFIX.QUEUE}:${key}`
        const tables = await getTableQueue(newKey)
        logger.info("--- getQueue :: tables :: ", tables);
        logger.info("--- getQueue :: newKey :: ", newKey);
        if (
            tables === null ||
            tables === undefined ||
            tables[0] === null ||
...
async function getQueueForBot(
    ...
    try {
        ...
        logger.error("--- getQueueForBot :: ERROR :: ", error);
        throw error;
    }
}
