...
async function matchMaking(data: matchMakingIf, socket: any,botTabledId:string, ack?: Function): Promise<void> {
    logger.info("===================>> Matchmaking <<=====================")
    const { userId } = socket.eventMetaData;
    const { lobbyId } = data;
    try {
        logger.info("-------->> Matchmaking :: botTabledId :: ", botTabledId);
        ...
        if (ack) {
            await insertNewPlayer(getUserData, socket,botTabledId, ack);
        }
