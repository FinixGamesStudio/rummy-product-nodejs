                    ...
                    if (playingTable != null) {
                        ...
                        if (userPlayingTable.playingStatus !== PLAYER_STATE.LEFT) {
                            ...
                            if (
                                ...
                                if(playingTable.isReferralCode && playingTable.isCreateRoom){

                                    logger.info(`----->> userDisconnect :: PRIVATETABLE.... ::`);
                                    return
                                ...
                                if (leaveStatus) {
                                    ...
                                    logger.info( `----->> userDisconnect :: leaveStatus  :: :: userData.OldLobbyId :: ${userData.OldLobbyId} && :: >>> playingTable.lobbyId :: ${playingTable.lobbyId}`);

                                   
                                    // set old tableIDs in userData
                                    if(!userData.isCreateRoom){
                                        logger.info( "----->> userDisconnect :: userData.isCreateRoom :: ::", userData.isCreateRoom );
                                        const userInfo = await setOldTableIdsHistory(userData, tableId);
                                        userData.oldTableId = userInfo.oldTableId;
                                       logger.info( "----->> userDisconnect :: userInfo :: :: >>", userInfo );
                                    ...
                                    logger.info( "----->> userDisconnect :: userData :: setUser", userData );
                                    // set rejoin history
                                ...
                                userData.OldLobbyId = playingTable.lobbyId;
                                logger.info( `----->> userDisconnect :: else :: userData.OldLobbyId :: :: ${userData.OldLobbyId} && :: >>> playingTable.lobbyId :: ${playingTable.lobbyId}`);

                                // set old tableIDs in userData
                                if(!userData.isCreateRoom){
                                    logger.info( `----->> userDisconnect :: playerLeave :: userData.isCreateRoom ::`,userData.isCreateRoom );
                                    const userInfo = await setOldTableIdsHistory(userData, tableId);
                                    userData.oldTableId = userInfo.oldTableId;
                                    logger.info( `----->> userDisconnect :: playerLeave :: userInfo ::`,userInfo );
                                }
