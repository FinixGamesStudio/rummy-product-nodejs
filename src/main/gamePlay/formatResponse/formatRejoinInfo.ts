import {
  ...
  userSeatKeyIF,
  userSeatKeyIf,
} from "../../interfaces/roundTableIf";
import Errors from "../../errors";
import {
  finishTimerStartPlayerDetailIf,
  formatRejoinInfoIf,
  formatScoreBoardInfoIf,
  popupDataIf,
  rejoinDataIf,
} from "../../interfaces/responseIf";
...
async function formatRejoinInfo(
  ...
  const {
    DEAL_NEW_GAME_TIMER,
    POINT_RUMMY_SCORE_BOARD_TIMER,
    DELAY_FOR_AUTO_SPLIT_IN_SCORE_BOARD_TIMER,
    TURN_TIMER,
    SECONDARY_TIMER,
  ...
  try {
    ...
    for await (const key of Object.keys(roundTableData.seats)) {
      if (Object.keys(roundTableData.seats[key]).length > 0) {
        const playerInfo = await getPlayerGamePlay(
          roundTableData.seats[key].userId,
          tableData._id
        );
        ...
        if (tableData.rummyType === RUMMY_TYPES.DEALS_RUMMY) {
          gameScore =
            playerInfo && playerInfo.gamePoints
              ? playerInfo.gamePoints + playerInfo.roundLostPoint
              : NUMERICAL.ZERO;
          ...
          gameScore =
            playerData && playerInfo.gamePoints
        ...
        const obj: userSeatKeyIF = {
          ...
          coins: roundTableData.seats[key].coins,
          avatarName: roundTableData.seats[key].avatarName,
      ...
      if (roundTableData.validDeclaredPlayer !== "") {
        if (
          roundTableData.seats[key].userId ===
          roundTableData.validDeclaredPlayer
        ) {
    ...
    if (data.isTurnStart) {
      if (roundTableData.currentTurn !== NULL) {
        for await (const key of Object.keys(roundTableData.seats)) {
          if (Object.keys(roundTableData.seats[key]).length > 0) {
            if (
              roundTableData.seats[key].userId === roundTableData.currentTurn
            ) {
    ...
    let isSeconderyTurnsRemain =
      playerData.remainSecondryTime > NUMERICAL.ZERO ? true : false;
    if (roundTableData.currentTurn !== playerData.userId) {
      const turnPlayData = await getPlayerGamePlay(
        roundTableData.currentTurn,
        tableData._id
      );
      if (turnPlayData) {
        isSeconderyTurnsRemain =
          turnPlayData.remainSecondryTime > NUMERICAL.ZERO ? true : false;
      }
    }
    ...
    if (
      ...
      popupData.msg =
        MESSAGES.POPUP.CENTER_TOAST_POPUP.INVALAID_DECLARE_POPUP_MESSAGE;
      isShowPopup = true;
    }
    ...
    if (
      ...
      popupData.msg =
        MESSAGES.POPUP.MIDDLE_TOAST_POP.FOR_VAILD_SEQUENCE_AND_DECLARE;
      isShowPopup = true;
    }

    // if  finishing timer start
    let finishTimerStartPlayerInfo: finishTimerStartPlayerDetailIf =
      {} as finishTimerStartPlayerDetailIf;
    ...
    let formatedScoreBoardRes: formatScoreBoardInfoIf =
      {} as formatScoreBoardInfoIf;
    ...
    if (data.isDeclareTimerStart) {
      ...
      if (declared) {
        ...
        if (tableData.rummyType === RUMMY_TYPES.POOL_RUMMY) {
          playersInfo = await poolRummyScoreBoadPlayerInfo(
            roundTableData.tableId,
            roundTableData
          );
        } else if (tableData.rummyType === RUMMY_TYPES.DEALS_RUMMY) {
          playersInfo = await dealRummyScoreBoadPlayerInfo(
            roundTableData.tableId,
            roundTableData
          );
        } else if (tableData.rummyType === RUMMY_TYPES.POINT_RUMMY) {
          playersInfo = await pointRummyScoreBoadPlayerInfo(
            roundTableData.tableId,
            roundTableData
          );
        }
    ...
    if (data.isDisplayScoreBoard) {
      ...
      if (tableData.rummyType === RUMMY_TYPES.POOL_RUMMY) {
        ...
        let playersInfo: UserInfoIf[] = await poolRummyScoreBoadPlayerInfo(
          roundTableData.tableId,
          roundTableData
        );
        const finalWinner = await poolRummyCheckFinalWinerInScoreBoard(
          playersInfo
        );
        ...
        logger.info(
          `------>> formatRejoinInfo :: finalWinner  1 :: :: `,
          finalWinner
        );
        logger.info(
          `------>> formatRejoinInfo :: playersInfo :: `,
          playersInfo
        );
        ...
        for await (const playerInfo of playersInfo) {
          if (playerInfo.tableId === roundTableData.tableId) {
            ...
            logger.info(
              `------>> formatRejoinInfo :: currentPlayerScoreBoardData :: `,
              currentPlayerScoreBoardData
            );
            logger.info(
              `------>> formatRejoinInfo :: playerInfo ||||| ----------->>>>:: `,
              playerInfo
            );
        ...
        logger.info(
          `------>> formatRejoinInfo :: finalWinner :: `,
          finalWinner
        );
        logger.info(
          `------>> formatRejoinInfo :: formatedScoreBoardRes ::`,
          formatedScoreBoardRes
        );

        let scoreBoardIdex: number = NUMERICAL.ZERO;
        for await (const playerScoreboardDetail of formatedScoreBoardRes.scoreBoradTable) {
          logger.info(
            `------>> formatRejoinInfo :: currentPlayerScoreBoardData 0 ------>>>>::`,
            currentPlayerScoreBoardData
          );
          if (
            playerScoreboardDetail.message === MESSAGES.MESSAGE.Eliminated ||
            playerScoreboardDetail.message ===
              MESSAGES.MESSAGE.REJOIN_AGAIN_GAME
          ) {
            formatedScoreBoardRes.scoreBoradTable[scoreBoardIdex].Status =
              PLAYER_STATE.ELIMINATED;
          }
          if (
            finalWinner.isFinalWinner &&
            playerScoreboardDetail.Status === PLAYER_STATE.WIN_ROUND
          ) {
            currentPlayerScoreBoardData.Status =
              currentPlayerScoreBoardData.Status === PLAYER_STATE.WIN_ROUND
                ? PLAYER_STATE.WIN
                : currentPlayerScoreBoardData.Status;
            formatedScoreBoardRes.scoreBoradTable[scoreBoardIdex].Status =
              PLAYER_STATE.WIN;
          ...
          if (roundTableData.isAutoSplit) {
            formatedScoreBoardRes.message =
              MESSAGES.MESSAGE.SCORE_BOARD_AUTO_SPLIT_MESSAGE;
          }

          scoreBoardIdex += NUMERICAL.ONE;
        }
        logger.info(
          `------>> formatRejoinInfo :: formatedScoreBoardRes :: 1 ::`,
          formatedScoreBoardRes
        );

        logger.info(
          `------>> formatRejoinInfo :: currentPlayerScoreBoardData 2 ------>>>>::`,
          currentPlayerScoreBoardData
        );

        formatedScoreBoardRes.message =
          currentPlayerScoreBoardData.message === MESSAGES.MESSAGE.Eliminated
            ? `${MESSAGES.MESSAGE.DEAL} ${tableData.currentRound} ${MESSAGES.MESSAGE.SCORE_BOARD_ELIMINATED_PLAYER_MESSAGE}`
            : formatedScoreBoardRes.message;
        formatedScoreBoardRes.message =
          finalWinner.isFinalWinner &&
          currentPlayerScoreBoardData.Status === PLAYER_STATE.WIN
            ? `${MESSAGES.MESSAGE.SCORE_BOARD_WINNER_MESSAGE}${finalWinner.winAmount}!`
            : formatedScoreBoardRes.message;
        formatedScoreBoardRes.timer =
          currentPlayerScoreBoardData.message === MESSAGES.MESSAGE.Eliminated
            ? NUMERICAL.MINUS_ONE
            : data.timer;
        formatedScoreBoardRes.timer =
          (finalWinner.isFinalWinner &&
            currentPlayerScoreBoardData.Status === PLAYER_STATE.WIN) ||
          roundTableData.isAutoSplit
            ? NUMERICAL.MINUS_ONE
            : formatedScoreBoardRes.timer;

        logger.info(
          `------>> formatRejoinInfo :: formatedScoreBoardRes :: 2 ::: ------->>>`,
          formatedScoreBoardRes
        );
        ...
        if (roundTableData.isWaitingForRejoinPlayer) {
          ...
          formatedScoreBoardRes.message =
            currentPlayerScoreBoardData.message === MESSAGES.MESSAGE.Eliminated
              ? MESSAGES.MESSAGE.REJOIN_GAME_PLAYER_SCORE_BOARD_MESSAGE
              : formatedScoreBoardRes.message;

          const rejoinGamePlayerData = await getPlayerGamePlay(
            roundTableData.eliminatedPlayers[NUMERICAL.ZERO],
            tableData._id
          );

          const timeDiff: number = timeDifference(
            new Date(),
            rejoinGamePlayerData.updatedAt,
            REJOINT_GAME_POPUP_TIMER
          );

          formatedScoreBoardRes.timer = !finalWinner.isFinalWinner
            ? timeDiff
            : formatedScoreBoardRes.timer;
        ...
        if (manualSplit && !roundTableData.isWaitingForRejoinPlayer) {
          ...
          if (roundTableData.isSplit) {
            ...
            for await (const player of splitPlayerData) {
              ...
              if (
                ...
                if (playerData.userId === playerInfo.userId) {
                  isResultSplit = true;
                  popupData.msg =
                    MESSAGES.MESSAGE.MANUAL_SPLIT_SINGLE_PLAYER_ACCEPED;
            ...
            const allEqual = splitPlayersStatus.every(
              (val) => val === SPLIT_STATE.YES
            );
            const isSplitStateIncludePedding = splitPlayersStatus.includes(
              SPLIT_STATE.PENDING
            );
            popupData.msg = allEqual
              ? `${MESSAGES.MESSAGE.SPLIT_WIN_MESSAGE}${playerData.splitDetails.amount}!`
              : playerData.splitDetails.splitStatus === SPLIT_STATE.NO
              ? MESSAGES.MESSAGE.MANUAL_SPLIT_REJECT_MESSAGE
              : !isSplitStateIncludePedding
              ? MESSAGES.MESSAGE.MANUAL_SPLIT_NOT_AGREE_MESSAGE
              : popupData.msg;

            data.timer =
              allEqual || !isSplitStateIncludePedding
                ? timeDifference(
                    new Date(),
      ...
      // deal rummy scoreBoard Data
        ...
        isScoreboard = true;
        const scoreBoardData = await getScoreBoardHistory(
          roundTableData.tableId
        );
        logger.info(
          "------ formatRejoinInfo :: scoreBoardData[`${roundTableData.tableId}`] : ",
          scoreBoardData[`Round${tableData.currentRound}`]
        );

        if (
          scoreBoardData &&
          scoreBoardData.hasOwnProperty(`Round${tableData.currentRound}`) &&
          "tableId" in scoreBoardData[`Round${tableData.currentRound}`]
        ) {
          ...
          isScoreboard = true;
          let msg = `Next round start in 0 seconds`;
          let playersInfo: UserInfoIf[] = await dealRummyScoreBoadPlayerInfo(
            roundTableData.tableId,
            roundTableData
          );
          let isLeaveBtn = false;
          ...
          const finalWinner = await dealRummyCheckFinalWinerInScoreBoard(
            playersInfo,
          ...
          for await (const player of formatedScoreBoardRes.scoreBoradTable) {
            if (finalWinner.isMultiWinner) {
              for await (const drawnPlayer of finalWinner.mulltiWinnerPlayerData) {
                if (drawnPlayer.userId === player.userId) {
                  formatedScoreBoardRes.scoreBoradTable[playerCount].Status =
                    PLAYER_STATE.DRAW;
            ...
            if (
              finalWinner.isFinalWinner &&
              player.userId === finalWinner.winnerPlayerData.userId
            ) {
              formatedScoreBoardRes.scoreBoradTable[playerCount].Status =
                PLAYER_STATE.WIN;
            }
            ...
            if (
              finalWinner.isFinalWinner &&
              player.Status === PLAYER_STATE.WIN_ROUND &&
              roundTableData.totalPlayers === NUMERICAL.ONE
            ) {
              formatedScoreBoardRes.scoreBoradTable[playerCount].Status =
                PLAYER_STATE.WIN;
      ...
      // point rummy scoreBoard Data
        ...
        data.timer = timeDifference(
          new Date(),
          roundTableData.updatedAt,
          Number(POINT_RUMMY_SCORE_BOARD_TIMER)
        );

        const scoreBoardData = await getScoreBoardHistory(playerData.userId);
        logger.info(
          "------ formatRejoinInfo :: scoreBoardData[`${playerData.userId}`] : ",
          scoreBoardData[`${playerData.userId}`]
        );

        if (
          scoreBoardData &&
          scoreBoardData.hasOwnProperty(`${playerData.userId}`) &&
          "tableId" in scoreBoardData[`${playerData.userId}`]
        ) {
          ...
          for await (const player of formatedScoreBoardRes.scoreBoradTable) {
            if (player.Status === PLAYER_STATE.WIN_ROUND) {
              formatedScoreBoardRes.scoreBoradTable[playersCount].Status =
                PLAYER_STATE.WIN;
          ...
          formatedScoreBoardRes.scoreBoradTable[winnerIndex].gameScore =
            formatedScoreBoardRes.scoreBoradTable[winnerIndex].Status ===
            PLAYER_STATE.WIN
              ? `+ ₹${winAmount * tableData.bootAmount}`
              : formatedScoreBoardRes.scoreBoradTable[winnerIndex].gameScore;
        }
      }
    }
    ...
    // for watching player
    if (
      roundTableData.seats[`s${playerData.seatIndex}`].userId ===
      playerData.userId
    ) {
      if (
        roundTableData.seats[`s${playerData.seatIndex}`].userStatus ===
        ...
        isShowPopup = true;
        popupData.msg =
          MESSAGES.POPUP.CENTER_TOAST_POPUP.YOU_ARE_SEAT_IN_WATCHING_MODE_PLEASE_WAITING_FOR_NEW_GAME_START;
      }
    }
    ...
    let winPrize: number =
      (roundTableData.tableState === TABLE_STATE.WAIT_FOR_PLAYER ||
        roundTableData.tableState === TABLE_STATE.WAITING_FOR_PLAYERS ||
        roundTableData.tableState === TABLE_STATE.ROUND_TIMER_STARTED) &&
      tableData.currentRound === NUMERICAL.ONE &&
      (roundTableData.maxPlayers === NUMERICAL.TWO ||
        roundTableData.totalPlayers <= NUMERICAL.TWO)
        ? Number(
            (
              NUMERICAL.TWO *
              tableData.bootAmount *
              (NUMERICAL.ONE - tableData.rake / 100)
            ).toFixed(NUMERICAL.TWO)
          )
        : tableData.winPrice;
    ...
    let Info: formatRejoinInfoIf = {
      ...
      userTotalTurnTimer: roundTableData.isSecondaryTurn
        ? SECONDARY_TIMER
        : TURN_TIMER,
      message: message,
      popupData: popupData,
      ...
      selfPlayerDetails: {
        ...
        profilePicture:
          roundTableData.seats[`s${playerData.seatIndex}`].profilePicture,
    ...
    logger.info(`------>> formatRejoinInfo :: Info :: `, Info);

    Info = await responseValidator.formatRejoinInfoValidator(Info);
    return Info;
  } catch (error) {
    console.log("CATCH_ERROR : formatRejoinInfo :: ", roundTableData, error);
    logger.error("CATCH_ERROR : formatRejoinInfo :: ", roundTableData, error);
