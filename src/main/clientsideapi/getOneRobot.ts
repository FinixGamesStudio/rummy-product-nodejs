...
const { GET_ONE_ROBOT, APP_KEY, APP_DATA } = config();
...
async function getOneRobot(tournamentId: string, token: string, tableId: string) {
    ...
    try {
        ...
        else if (getOneRobotDetail.data.isUseBotAvailable === false) {
            throw new Errors.InvalidInput("isUseBotAvailable false");
        }
