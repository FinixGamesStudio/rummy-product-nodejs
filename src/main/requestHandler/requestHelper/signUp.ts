...
import { BOT_CONSTANT, EVENTS, MESSAGES, NUMERICAL, PRODUCTION } from "../../../constants";
...
import { checkIsValidTokenHandle } from "../../../cms/helper/checkIsValidToken";
import { getRandomBotType, getRandomNumber } from "../../common/botcCommon/botCount";
...
async function signUpHandler(
    ...
    try {
        ...
        // let checkMaintanenceData = await checkMaintanence(socket.authToken, socket.id, signUpData.userId);
        ...
        // let isValidUserData = await verifyUserProfile(socket.authToken, signUpData.gameId, socket.id, signUpData.userId);
        // logger.info(signUpData.userId, "isValidUserData :: >> ", isValidUserData);

        const userToken = socket?.authToken || signUpData.accessToken;
        signUpData.accessToken = userToken;
        logger.info(signUpData.userId, "signUpData :: >> userToken :: >> ", userToken, "signUpData.accessToken :: >>", signUpData.accessToken);

        // const { isValidUser, user } = await checkIsValidTokenHandle(userToken);
        // if (!isValidUser) {
        ...
        //     await firstTimeIntrection(signUpData.gameId, socket.authToken, socket.id, signUpData.userId, signUpData.gameModeId);
        ...
        if (error instanceof Errors.InvalidInput) {
            ...
            commonEventEmitter.emit(EVENTS.SHOW_POP_SOCKET_EVENT, {
                ...
                data: {
                    isPopup: true,
                    popupType: MESSAGES.POPUP.COMMAN_TOAST_POPUP.INVALID_INPUT_POPUP_MESSAGE ?
                        MESSAGES.POPUP.COMMAN_TOAST_POPUP.INVALID_INPUT_POPUP_MESSAGE : MESSAGES.ALERT_MESSAGE.TYPE.COMMON_POPUP,
                    title: nonProdMsg,
                    message: error && error.message && typeof error.message === 'string'
                        ? error.message
                        : MESSAGES.GRPC_ERRORS.COMMON_ERROR,
