...
import { botAutoDeclareIf, botCardPickTurnTimerIf, botFinishCardTimerIf, botWinTimerIf } from "../interfaces/botIf";
import findBot from "../gamePlay/playBot/findBot";
import { pickCardForBot } from "../gamePlay/playBot/BotTurn/pickCardforBot";
import { discardCardForBot } from "../gamePlay/playBot/BotTurn/discardCardForBot";
import { finishCardForBot } from "../gamePlay/playBot/BotTurn/finishCardForBot";
import { winforBot } from "../gamePlay/playBot/BotTurn/winforBot";
import { botAutoDeclare } from "../common/botcCommon/botAutoDeclare";
...
function grtUserProfile(payload: any) {
    const {data,socket } = payload;
    const responseData = {
      en: EVENTS.GET_USER_PROFILE,
      data,
    };
    if (socket) {
      sendEventToClient(socket, responseData);
  ...
  function referralCodeEvent(payload: any) {
    const { tableId, data,socket } = payload;
    const responseData = {
      en: EVENTS.SHARE_REFERRAL_CODE,
      data,
    };
    if (socket) {
      sendEventToClient(socket, responseData);
...
commonEventEmitter.on(EVENT_EMITTER.GET_USER_PROFILE,grtUserProfile);

commonEventEmitter.on(EVENTS.SHARE_REFERRAL_CODE, referralCodeEvent);

//bot seat on table
commonEventEmitter.on(
    EVENT_EMITTER.FIND_BOT_TIMER_EXPIRED,
    (res: { tableId: string , userId : string}) => {
        logger.info('find bot res ::: -->>', res);
        findBot(res.tableId, res.userId);
    },
);

//bot turn on table
commonEventEmitter.on(
    EVENT_EMITTER.BOT_TURN_TIMER_EXPIRED,
    (res : startTurnTimerQueueIf) => {
        logger.info('pick Card Turn bot res ::: ', res);
        pickCardForBot(res.tableId, res.currentTurnPlayerId);
    },
);

//bot pick card after discard card turn on table
commonEventEmitter.on(
    EVENT_EMITTER.BOT_CARD_PICK_TURN_TIMER_EXPIRED,
    (res : botCardPickTurnTimerIf) => {
        logger.info('discard Card Turn bot res ::: ', res);
        discardCardForBot(res.tableId, res.currentTurn);
    },
);

//bot finish card turn on table
commonEventEmitter.on(
    EVENT_EMITTER.BOT_FINISH_CARD_TIMER_EXPIRED,
    (res : botFinishCardTimerIf) => {
        logger.info('finish Card Turn bot res ::: ', res);
        finishCardForBot(res.tableId, res.currentTurn);
    },
);

//bot win timer on table
commonEventEmitter.on(
    EVENT_EMITTER.BOT_WIN_TIMER_EXPIRED,
    (res : botWinTimerIf) => {
        logger.info('win bot timer res ::: ', res);
        winforBot(res.tableId);
    },
);

// bot auto decalre 
commonEventEmitter.on(
    EVENT_EMITTER.BOT_AUTO_DECLARE,
    (res : botAutoDeclareIf) => {
        logger.info('Auto Declare bot res ::: ', res);
        botAutoDeclare(res);
    },
);